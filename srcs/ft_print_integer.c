...
int		check_width(t_flags list_flags, int num_int, int len)
{
	...
	{
		if (num_int < 0)
			count += add_forward_char(' ', \
			list_flags.width - list_flags.precision - 1);
		else
			count += add_forward_char(' ', \
			list_flags.width - list_flags.precision);
	}
	else
...
int		check_minus(t_flags list_flags, int num_int, int len, char *arg)
{
	...
	if (list_flags.minus || (list_flags.width < 0 && num_int > 0))
	{
		...
		if (list_flags.width <= 0 && ft_abs(list_flags.width) >= len)
			...
			((list_flags.precision > len) ? list_flags.precision : len));
	}
	else if (!list_flags.minus)
	{
		...
		if (num_int <= 0 && list_flags.width <= 0)
			count += add_forward_char(' ', ft_abs(list_flags.width) - \
			((list_flags.precision > len) ? list_flags.precision + 1 : len));
	}
	return (count);
}
